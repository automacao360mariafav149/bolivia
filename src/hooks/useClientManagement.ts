...
export const useClientManagement = () => {
  ...
  const handleAddContact = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        const endpoints = savedEndpoints ? JSON.parse(savedEndpoints) : {};
        const criaUsuarioUrl = endpoints.criaUsuario || 'https://n8n_mcp.automacaodigital360.com/webhook/cria_usuario';
        
        try {
          await fetch(criaUsuarioUrl, {
            method: 'POST',
            headers: {
  ...
  const handleEditContact = async () => {
    ...
    try {
      ...
      const savedEndpoints = localStorage.getItem('webhookEndpoints');
      const endpoints = savedEndpoints ? JSON.parse(savedEndpoints) : {};
      const editaUsuarioUrl = endpoints.editaUsuario || 'https://n8n_mcp.automacaodigital360.com/webhook/edita_usuario';
      
      try {
        await fetch(editaUsuarioUrl, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const handleDeleteContact = async () => {
    ...
    try {
      ...
      const savedEndpoints = localStorage.getItem('webhookEndpoints');
      const endpoints = savedEndpoints ? JSON.parse(savedEndpoints) : {};
      const excluiUsuarioUrl = endpoints.excluiUsuario || 'https://n8n_mcp.automacaodigital360.com/webhook/exclui_usuario';
      
      try {
        await fetch(excluiUsuarioUrl, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const handlePauseDurationConfirm = async (duration: number | null) => {
    ...
    try {
      ...
      const savedEndpoints = localStorage.getItem('webhookEndpoints');
      const endpoints = savedEndpoints ? JSON.parse(savedEndpoints) : {};
      const enviaMensagemUrl = endpoints.enviaMensagem || 'https://n8n_mcp.automacaodigital360.com/webhook/envia_mensagem';
      
      const response = await fetch(enviaMensagemUrl, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
